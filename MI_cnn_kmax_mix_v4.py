...
Only restrict that the total number of positive cases should less than some values
The sentence similarity are placed on all sentences
"""
...
class GICF(object):
    ...
    def run_experiment(self, dataset, word_embedding, exp_name):
        ...
        # only place cost on positive bag
        penal_cost = T.sum(pos_cost * y) / T.maximum(nn.as_floatX(1e-7), T.sum(y))
