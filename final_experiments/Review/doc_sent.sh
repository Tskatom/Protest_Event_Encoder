...
vocab_fn=data/english_review.trn-${max_num}.vocab
...
pretrained_fn=${home}/workspace/Protest_Event_Encoder/data/glove.6B.${word_dm}d.txt
...
for i in `seq 0 4`;
do
    exp_name=doc_sent_max_F100_fold_${i}
    log_fn=./log/${exp_name}.log
    perf_fn=./results/
    param_fn=./doc_sent_max_param.json
    python $model_exe --prefix ${home}/workspace/Protest_Event_Encoder/data/noRats/${i}/Imdb_review --sufix sent_cat --word2vec $vec_trained_fn --dict_fn ${home}/workspace/Protest_Event_Encoder/data/sent_cat.dic --max_sens 40 --max_words 70 --padding 2 --exp_name $exp_name --max_iter 100 --batch_size 50 --log_fn $log_fn --perf_fn $perf_fn --param_fn $param_fn
