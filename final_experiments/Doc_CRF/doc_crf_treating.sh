...
model_exe=${home}/workspace/Protest_Event_Encoder/CNN_CRF.py
...
$prep_exe gen_vocab input_fn=./data/treating_tokens.lst vocab_fn=$vocab_fn max_vocab_size=$max_num \
    min_word_count=$min_word_count $options WriteCount

echo Construct two set of vocabulary embedding: ramdom and pretrained \n
vec_trained_fn=${home}/workspace/ssd/data/treating_trained_w2v_${word_dm}.pkl
vec_random_fn=${home}/workspace/ssd/data/treating_random_w2v_${word_dm}.pkl
...
exp_name=doc_crf_fold_treating
log_fn=./log/${exp_name}.log
perf_fn=./results/
param_fn=./doc_crf_param.json
python $model_exe --prefix ${home}/workspace/Protest_Event_Encoder/data/treating_single_label/spanish_protest --sufix_pop pop_cat --sufix_type type_cat --word2vec $vec_trained_fn --dict_pop_fn ${home}/workspace/Protest_Event_Encoder/data/pop_cat.dic --dict_type_fn ${home}/workspace/Protest_Event_Encoder/data/type_cat.dic --max_sens 30 --max_words 70 --padding 2 --exp_name $exp_name --max_iter 100 --batch_size 100 --log_fn $log_fn --perf_fn $perf_fn --param_fn $param_fn
