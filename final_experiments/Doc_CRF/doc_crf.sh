...
for i in `seq 0 4`;
do
    exp_name=doc_crf_fold_${i}_b50
    log_fn=./log/${exp_name}.log
    perf_fn=./results/
    param_fn=./doc_crf_param.json
    python $model_exe --prefix ${home}/workspace/Protest_Event_Encoder/data/single_label/${i}/spanish_protest --sufix_pop pop_cat --sufix_type type_cat --word2vec $vec_trained_fn --dict_pop_fn ${home}/workspace/Protest_Event_Encoder/data/pop_cat.dic --dict_type_fn ${home}/workspace/Protest_Event_Encoder/data/type_cat.dic --max_sens 30 --max_words 70 --padding 2 --exp_name $exp_name --max_iter 100 --batch_size 50 --log_fn $log_fn --perf_fn $perf_fn --param_fn $param_fn
