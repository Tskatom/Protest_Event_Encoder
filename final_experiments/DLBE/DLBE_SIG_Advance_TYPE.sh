...
for i in `seq 0 4`;
do
    exp_name=DLBE_MLT_w${word_dm}_k${k}_d${d}_fold_${i}_b100_N23_Advance_Auto_Sencost_TYPE
    log_fn=./log/${exp_name}.log
    perf_fn=./results/
    param_fn=./DLBE_param_d${d}_type.json
    python $model_exe --prefix ../../data/new_single_label/${i}/spanish_protest --sufix_pop pop_cat --sufix_type type_cat --word2vec $vec_trained_fn --dict_pop_fn ../../data/pop_cat.dic --dict_type_fn ../../data/type_cat.dic --max_sens 20 --max_words 50 --padding 2 --exp_name $exp_name --max_iter 50 --batch_size 128 --log_fn $log_fn --perf_fn $perf_fn --param_fn $param_fn --top_k $k --data_type json --flag type
done
