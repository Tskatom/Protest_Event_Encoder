...
class GICF(object):
    ...
    def run_experiment(self, dataset, word_embedding, exp_name):
        ...
        dropout_sent_vec = dropout_sent_vec.reshape((x.shape[0], x.shape[1], sent_vec_dim))
        ...
        con_dropout_sent_vec = T.concatenate([dropout_sent_vec, T.tile(dropout_doc_vec, [1, x.shape[1]]).reshape((x.shape[0], x.shape[1], doc_vec_dim))], axis=2).reshape((x.shape[0]*x.shape[1], sent_vec_dim + doc_vec_dim))
        
        con_sent_vec = T.concatenate([sent_vec, T.tile(doc_vec, [1, x.shape[1]]).reshape((x.shape[0], x.shape[1], doc_vec_dim))], axis=2).reshape((x.shape[0]*x.shape[1], sent_vec_dim + doc_vec_dim))
        ...
        drop_sent_prob = T.nnet.sigmoid(T.dot(con_dropout_sent_vec, sen_W) + sen_b)
