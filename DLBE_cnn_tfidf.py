...
def run_cnn(exp_name, tfidf_dataset,
        dataset, embedding,
        log_fn, perf_fn,
        k=0,
        emb_dm=100,
    ...
    hidden_units[0] = num_maps * len(filter_hs) + 20612
    ...
    train_tfidf = theano.shared(np.asarray(tfidf_dataset[0],
        dtype=theano.config.floatX), borrow=True)
    test_tfidf = theano.shared(np.asarray(tfidf_dataset[1],
        dtype=theano.config.floatX), borrow=True)
...
def main():
    ...
    tfidf_dataset = load_tfidf(prefix)
    ...
    run_cnn(exp_name, tfidf_dataset,
            digit_dataset, embedding,
            log_fn, perf_fn,
            k=top_k,
