...
def run_cnn(exp_name,
    ...
    for i in xrange(len(filter_hs)):
        filter_shape = (num_maps / 2, 1, filter_hs[i], emb_dm)
        #pool_size = (input_height - filter_hs[i] + 1, 1)
        pool_size1 = (filter_hs[i], 1)
        conv_layer = nn.ConvPoolLayer(rng, input=layer0_input, 
                input_shape=None,
                filter_shape=filter_shape,
                pool_size=pool_size1, activation=activation)
        """
        # second layer conv
        filter_shape2 = (num_maps, num_maps/2, filter_hs[i], 1)
        pool_size2 = ((input_height - filter_hs[i] + 1) / filter_hs[i], 1)
        conv_layer2 = nn.ConvPoolLayer(rng, input=conv_layer.output,
                input_shape=None,
                filter_shape=filter_shape2,
                pool_size=pool_size2, activation=activation)
        """
    ...
    while (epoch < n_epochs) and not done_loop:
        ...
        if epoch % 1 == 0:
            ...
            #valid_preds = np.concatenate([valid_pred(i) for i in xrange(n_valid_batches)])
            #valid_score = compute_score(cpu_val_y, valid_preds)
            print '========'
            with open(os.path.join(perf_fn, "%s_%d.pred" % (exp_name, epoch)), 'w') as epf:
                for p in test_preds:
                    epf.write("%d\n" % int(p))
            #message = "Epoch %d test perf %f train perf %f" % (epoch, test_score, valid_score)
