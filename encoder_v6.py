...
def train_encoder():
    ...
    embedding = shared(value=np.asarray(embedding, dtype=theano.config.floatX), 
    	borrow=True)
    ...
    W0 = theano.shared(np.asarray(rng.uniform(-w_bound, w_bound,size=filter_shape),
    ...
    filter_shape1 = [64, 1, 64, 1]
    f_in = 1 * 64 * 1
    f_out = 64 * 64 * 1
    w_bound = np.sqrt(6./(f_in + f_out))
    W1 = theano.shared(np.asarray(rng.uniform(-w_bound, w_bound,size=filter_shape1),
    	dtype=theano.config.floatX), borrow=True)
    b1 = theano.shared(np.zeros((filter_shape1[0],), dtype=theano.config.floatX),
    	borrow=True)
    doc_layer_input = sen_conv_output.dimshuffle('x','x',1,0)
    doc_conv = T.nnet.conv.conv2d(doc_layer_input, filter_shape1, border_mode="valid")
    ...
    f_out = 32
    w_bound = np.sqrt(6./(f_in + f_out))
    W2 = theano.shared(np.asarray(rng.uniform(-w_bound, w_bound,size=(f_in, f_out)),
    	dtype=theano.config.floatX), borrow=True)
    ...
    f_out = num_pop_class
    w_bound = np.sqrt(6./(f_in + f_out))
    W3 = theano.shared(np.asarray(rng.uniform(-w_bound, w_bound,size=(f_in, f_out)),
    	dtype=theano.config.floatX), borrow=True)
    b3 = theano.shared(np.zeros((filter_shape1[0],), dtype=theano.config.floatX),
    	borrow=True)
    pop_preact = T.dot(f_out, W3) + b3
    ...
    test_func= function(inputs=[doc, t1_y], outputs=pop_error)
